...
export function AppHeader({ authControls }: { authControls?: ReactNode }) {
  ...
  const load = useCallback(async () => { try { const res = await fetch("/api/notifications", { cache: "no-store" }); if (res.ok) { const data = (await res.json()) as { notifications?: AgentNotification[] }; if (Array.isArray(data.notifications)) setNotifications(data.notifications); } } catch { /* keep the shell usable offline */ } finally { setLoaded(true); } }, []);
  useEffect(() => { if (!loaded) void load(); }, [loaded, load]);
  const onDismiss = useCallback((id: string) => { setNotifications((prev) => prev.map((item) => item.id === id ? { ...item, read: true } : item)); void fetch("/api/notifications", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) }); }, []);

  return <>
    <header className="sticky top-0 z-30 flex min-h-20 items-center justify-between gap-4 border-b border-[var(--color-line)] bg-[color-mix(in_srgb,var(--color-bg)_76%,transparent)] px-5 py-4 backdrop-blur-2xl sm:px-8">
    ...
    <NotificationDrawer open={drawerOpen} notifications={notifications} onClose={() => setDrawerOpen(false)} onDismiss={onDismiss} />
  </>;
}
